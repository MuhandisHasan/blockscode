...
function moveUp(curPosition,collisons)
{
	...
	if (collisons.includes(blockId)) {
		...
		car.style.top = getPosition(document.getElementById(blockId)).top + "px";
		car.style.left = getPosition(document.getElementById(blockId)).left + "px";
		setTimeout(function(){alert("Tebrikler!"); location.reload();},600);
...
function addCode(id) {
	...
	if(usableLeft == 0){
		...
		alert("Daha fazla kod kullanamaz !");

	}else if (id == "loop") {

		var times = +prompt("Kodlar kaç defa tekrarlayacak ?");

		console.log(times);

		if (isNaN(times) || times == 0 || times == 1) {

			alert("1 den fazla bir sayı girin!");
		...
		workSpace.innerHTML += '<div class="code">Tekrarlama Bitişi</div>';
